...
const mockPosts: BlogPost[] = [
  { id: "1", title: "أفضل 10 نصائح لتجربة تسوق إلكتروني سلسة", author: "أحمد محمد", category: "نصائح", content: "محتوى المقال...", status: "published", publishedAt: "2026-07-10", comments: 24, views: 1850 },
  { id: "2", title: "كيف تختار المنتج المناسب عبر الإنترنت", author: "سارة العلي", category: "دليل المشتري", content: "محتوى المقال...", status: "published", publishedAt: "2026-07-05", comments: 18, views: 2340 },
  { id: "3", title: "أحدث صيحات الموضة في 2026", author: "نورة الخليفة", category: "أزياء", content: "محتوى المقال...", status: "published", publishedAt: "2026-06-28", comments: 31, views: 4200 },
  { id: "4", title: "دليل شامل للعناية بالأجهزة الإلكترونية", author: "خالد الراشد", category: "تكنولوجيا", content: "محتوى المقال...", status: "published", publishedAt: "2026-06-20", comments: 12, views: 1560 },
  { id: "5", title: "أسرار العناية بالبشرة في فصل الصيف", author: "منى الحربي", category: "جمال وصحة", content: "محتوى المقال...", status: "draft", publishedAt: "", comments: 0, views: 0 },
  { id: "6", title: "كيف تبني علامة تجارية ناجحة", author: "عمر السعيد", category: "أعمال", content: "محتوى المقال...", status: "draft", publishedAt: "", comments: 0, views: 0 },
];
...
export default function BlogPage() {
  ...
  const handleSave = useCallback(() => {
    ...
    if (editTarget) {
      update(editTarget.id, { title: formTitle, author: formAuthor, category: formCategory, content: formContent, status: formStatus, ...(formStatus === "published" && !editTarget.publishedAt ? { publishedAt: now } : {}) });
      success("تم التحديث", `تم تحديث مقال "${formTitle}" بنجاح`);
    } else {
      add({ id: generateId(), title: formTitle, author: formAuthor, category: formCategory, content: formContent, status: formStatus, publishedAt: formStatus === "published" ? now : "", comments: 0, views: 0 });
      success("تمت الإضافة", `تم إنشاء مقال "${formTitle}" بنجاح`);
    }
    setModalOpen(false);
    setEditTarget(null);
  }, [formTitle, formAuthor, formCategory, formContent, formStatus, editTarget, add, update, success, showError]);
  ...
  const columns = [
    ...
    {
      key: "author" as const, label: "المؤلف", sortable: true,
    ...
    { key: "publishedAt" as const, label: "تاريخ النشر", sortable: true, render: (value: unknown) => value ? formatDate(String(value)) : "—" },
    { key: "comments" as const, label: "التعليقات", sortable: true, render: (value: unknown) => <div className="flex items-center gap-1 text-text-secondary"><MessageCircle size={14} /><span>{Number(value)}</span></div> },
    { key: "views" as const, label: "المشاهدات", sortable: true, render: (value: unknown) => Number(value).toLocaleString("ar-SA") },
    {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="المدونة" subtitle="إدارة مقالات المدونة" actions={<Button icon={<Plus size={16} />} onClick={openCreate}>مقال جديد</Button>} />
      ...
      <DataTable columns={columns} data={paginatedData} emptyMessage="لا توجد مقالات" rowKey="id" sortable pagination={{ currentPage: page, totalPages, totalItems, itemsPerPage: perPage, onPageChange: setPage, onItemsPerPageChange: setPerPage }} striped />
        ...
        <Modal open onClose={() => setPreviewPost(null)} title="معاينة المقال" size="lg">
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4 text-sm text-text-muted">
              <span>المؤلف: {previewPost.author}</span>
            ...
            <div className="flex items-center gap-4 text-sm text-text-muted border-t border-border pt-4">
              <span>{previewPost.views} مشاهدة</span>
              <span>{previewPost.comments} تعليق</span>
            </div>
      ...
      {modalOpen && (
        <Modal open onClose={() => { setModalOpen(false); setEditTarget(null); }} title={editTarget ? "تعديل المقال" : "مقال جديد"} size="lg">
          <div className="space-y-4">
