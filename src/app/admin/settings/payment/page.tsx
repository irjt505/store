...
import { CreditCard, Smartphone, Wallet, Banknote, Building2, Settings2, Send } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {methods.map((method, idx) => {
          ...
          return (
            <motion.div key={method.id} initial="hidden" animate="visible" variants={fadeUp} transition={{ delay: idx * 0.04 }}>
              <Card padding="md" className={cn("transition-all", method.active ? "border-border" : "opacity-70")}>
                <div className="flex items-start justify-between mb-4">
                  <div className={cn("flex h-12 w-12 items-center justify-center rounded-xl", method.bg)}>
                    <span className={method.color}><Icon size={24} /></span>
                  </div>
